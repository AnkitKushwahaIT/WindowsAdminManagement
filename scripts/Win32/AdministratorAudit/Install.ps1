...
# ==========================================
# Administrator Audit - Install
# Intune Win32 app / SYSTEM / 64-bit PowerShell
# ==========================================

# Configuration - keep values identical in Install.ps1 and Detection.ps1.
$AppVersion = '1.0.0'
$PackageName = 'AdministratorAudit'
$ReportMaxAgeHours = 24
$LogFolder = 'C:\CompanyIT'
$LogFile = Join-Path $LogFolder "AdministratorAudit.log"
$StateFolder = Join-Path $env:ProgramData 'WindowsAdminManagement\Win32\AdministratorAudit'
...
try {
    ...
    Initialize-State
    $Members = @(Get-Members)
    Write-Report ([ordered]@{ Count=$Members.Count; Members=$Members })
    Save-Receipt
