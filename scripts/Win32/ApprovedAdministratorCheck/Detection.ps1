...
# ==========================================
# Approved Administrator Check - Detection
...
try {
    ...
    if (@($Status | Where-Object { -not $_.Approved }).Count) { throw 'Unexpected administrator membership detected.' }
    Test-FreshReport
    Write-Log '========== Detection Completed Successfully =========='
    Write-Output 'Detected: Approved Administrator Check.'
    exit 0
}
catch {
