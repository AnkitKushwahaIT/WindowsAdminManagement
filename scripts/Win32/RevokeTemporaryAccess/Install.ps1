...
# ==========================================
# Revoke Temporary Access - Install
# Intune Win32 app / SYSTEM / 64-bit PowerShell
# ==========================================

# Configuration - keep values identical in Install.ps1 and Detection.ps1.
$AppVersion = '1.0.0'
$PackageName = 'RevokeTemporaryAccess'
# Use a new RequestId for each approved action. Configure both scripts identically.
$TargetUserSid = ''
$RequestId = ''
$LogFolder = 'C:\CompanyIT'
$LogFile = Join-Path $LogFolder "RevokeTemporaryAccess.log"
$StateFolder = Join-Path $env:ProgramData 'WindowsAdminManagement\Win32\RevokeTemporaryAccess'
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest
$Policy = [ordered]@{ Package=$PackageName; Version=$AppVersion; TargetUserSid=$TargetUserSid; RequestId=$RequestId }
...
try {
    ...
    if ($RequestId -notmatch '^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$') { throw 'Configure a unique RequestId using letters, digits, underscores or hyphens.' }
    Import-Module ScheduledTasks -ErrorAction Stop
    ...
    if (Test-Path -LiteralPath $ReceiptFile) {
        $null = Read-Receipt
        Write-Log 'This request already completed. Later access is not revoked again.'
    }
    else {
        $Members = @(Get-Members)
        $Current = @($Members | Where-Object { $_.SID -eq $TargetUserSid })
        if ($Current.Count -and ($Current[0].Name -notmatch '^AzureAD\\' -or $Current[0].ObjectClass -ne 'User')) { throw 'Target is not a direct Entra user entry.' }
        $Tasks = @(Get-ScheduledTask -TaskPath '\' -ErrorAction Stop | Where-Object { $_.TaskName -eq "TempAdminRemoval_$TargetUserSid" })
        if ($Current.Count -and $Tasks.Count -eq 0) { throw 'Target has no repository removal task; refusing to revoke potentially permanent access.' }
        if ($Tasks.Count) {
            $Removal = Get-RemovalTask -Sid $TargetUserSid -AllowExpired
            if ($Current.Count) {
                Write-Log "Removing temporary administrator SID: $TargetUserSid"
                Remove-LocalGroupMember -SID 'S-1-5-32-544' -Member $TargetUserSid -Confirm:$false -ErrorAction Stop
            }
            if (@(Get-Members | Where-Object { $_.SID -eq $TargetUserSid }).Count) { throw 'Membership remains after removal.' }
            Write-Log 'Removal verified. Deleting the corresponding scheduled task.'
            Unregister-ScheduledTask -TaskName $Removal.Name -TaskPath '\' -Confirm:$false -ErrorAction Stop
        }
        if (@(Get-Members | Where-Object { $_.SID -eq $TargetUserSid }).Count -or @(Get-ScheduledTask -TaskPath '\' -ErrorAction Stop | Where-Object { $_.TaskName -eq "TempAdminRemoval_$TargetUserSid" }).Count) { throw 'Revocation verification failed.' }
        Write-Report ([ordered]@{ RequestId=$RequestId; TargetUserSid=$TargetUserSid; Result='Direct membership and removal task absent' })
        Save-Receipt
    }
