...
# ==========================================
# Temporary Access Check - Detection
# Intune Win32 app / SYSTEM / 64-bit PowerShell
# ==========================================

# Configuration - keep values identical in Install.ps1 and Detection.ps1.
$AppVersion = '1.0.0'
$PackageName = 'TemporaryAccessCheck'
# Direct Entra users approved permanently do not require a temporary removal task.
$PermanentEntraAdminSids = @()
$ReportMaxAgeHours = 24
$LogFolder = 'C:\CompanyIT'
$LogFile = Join-Path $LogFolder "TemporaryAccessCheck.log"
$StateFolder = Join-Path $env:ProgramData 'WindowsAdminManagement\Win32\TemporaryAccessCheck'
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest
$Policy = [ordered]@{ Package=$PackageName; Version=$AppVersion; PermanentEntraAdminSids=$PermanentEntraAdminSids; ReportMaxAgeHours=$ReportMaxAgeHours }
...
try {
    ...
    Assert-Sids $PermanentEntraAdminSids
    ...
    $Status = @(Get-TemporaryStatus -Members @(Get-Members) -PermanentSids $PermanentEntraAdminSids)
    if (@($Status | Where-Object { -not $_.Valid }).Count) { throw 'Temporary access requires attention.' }
    Test-FreshReport
    Write-Log '========== Detection Completed Successfully =========='
    Write-Output 'Detected: Temporary Access Check.'
    exit 0
}
catch {
