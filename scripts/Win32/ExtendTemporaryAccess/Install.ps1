...
# ==========================================
# Extend Temporary Access - Install
...
try {
    ...
    if (Test-Path -LiteralPath $ReceiptFile) {
        $null = Read-Receipt
        Write-Log 'This extension request already completed; no renewal or re-grant performed.'
    }
    else {
        if ($RequestedExpiry.UtcDateTime -le [datetime]::UtcNow -or $RequestedExpiry.UtcDateTime -gt [datetime]::UtcNow.AddDays(365)) { throw 'Requested expiry must be in the future and no more than 365 days away.' }
        $Current = @(Get-Members | Where-Object { $_.SID -eq $TargetUserSid })
        if ($Current.Count -ne 1 -or $Current[0].Name -notmatch '^AzureAD\\' -or $Current[0].ObjectClass -ne 'User') { throw 'Target must still have direct Entra administrator membership.' }
        $Removal = Get-RemovalTask $TargetUserSid
        if ([string]$Removal.Task.State -eq 'Running') { throw 'Removal is already running; extension is refused.' }
        if ($RequestedExpiry.UtcDateTime -lt $Removal.Expiry.UtcDateTime) { throw 'Requested expiry is earlier than the existing expiry.' }
        if ($RequestedExpiry.UtcDateTime -gt $Removal.Expiry.UtcDateTime) {
            Write-Log "Extending $TargetUserSid from $($Removal.Expiry.ToString('o')) to $NewExpiryUtc"
            $Trigger = New-ScheduledTaskTrigger -Daily -DaysInterval 1 -At $RequestedExpiry.LocalDateTime
            # Alter scheduling only; preserve the existing action, principal and settings.
            Set-ScheduledTask -TaskName $Removal.Name -TaskPath '\' -Trigger $Trigger -Description "Temporary direct admin removal for $TargetUserSid. Expires $NewExpiryUtc." -ErrorAction Stop | Out-Null
        }
        $Verified = Get-RemovalTask $TargetUserSid
        if ([math]::Abs(($Verified.Expiry.UtcDateTime - $RequestedExpiry.UtcDateTime).TotalSeconds) -gt 1) { throw 'Expiry verification failed.' }
        if (@(Get-Members | Where-Object { $_.SID -eq $TargetUserSid }).Count -ne 1) { throw 'Target membership disappeared during extension.' }
        Write-Report ([ordered]@{ RequestId=$RequestId; TargetUserSid=$TargetUserSid; ExpiryUtc=$NewExpiryUtc })
        Save-Receipt
    }
    Write-Log '========== Install Completed Successfully =========='
    Write-Output 'Completed: Extend Temporary Access.'
    exit 0
}
catch {
