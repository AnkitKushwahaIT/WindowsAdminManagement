...
# ==========================================
# Break Glass Account Health - Detection
# Intune Win32 app / SYSTEM / 64-bit PowerShell
# ==========================================

# Configuration - keep values identical in Install.ps1 and Detection.ps1.
$AppVersion = '1.0.0'
$PackageName = 'BreakGlassHealth'
$UserName = 'LocalAdmin'
$ReportMaxAgeHours = 24
$LogFolder = 'C:\CompanyIT'
$LogFile = Join-Path $LogFolder "BreakGlassHealth.log"
$StateFolder = Join-Path $env:ProgramData 'WindowsAdminManagement\Win32\BreakGlassHealth'
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest
$Policy = [ordered]@{ Package=$PackageName; Version=$AppVersion; UserName=$UserName; ReportMaxAgeHours=$ReportMaxAgeHours }
...
function Get-AccountHealth {
    $Users = @(Get-LocalUser -ErrorAction Stop | Where-Object { $_.Name -eq $UserName })
    if ($Users.Count -ne 1) { return [pscustomobject]@{ UserName=$UserName; Exists=$false; Enabled=$false; Administrator=$false; Healthy=$false } }
    $User = $Users[0]
    $IsAdmin = @(Get-Members | Where-Object { $_.SID -eq $User.SID.Value }).Count -eq 1
    [pscustomobject]@{ UserName=$UserName; Exists=$true; Enabled=[bool]$User.Enabled; Administrator=$IsAdmin; Healthy=([bool]$User.Enabled -and $IsAdmin) }
}
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($UserName)) { throw 'Configure UserName.' }
    ...
    $Health = Get-AccountHealth
    if (-not $Health.Healthy) { throw 'Break-glass account is absent, disabled or not a direct administrator.' }
    Test-FreshReport
    Write-Log '========== Detection Completed Successfully =========='
    Write-Output 'Detected: Break Glass Account Health.'
    exit 0
}
catch {
